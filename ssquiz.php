...
function ssquiz_install () {
	...
	if( $installed_ver != $ssquiz_db_version ) {
		$sql = "
		CREATE TABLE {$wpdb->prefix}ssquiz_quizzes (
		...
		CREATE TABLE {$wpdb->prefix}ssquiz_questions (
			id mediumint(9) UNSIGNED NOT NULL AUTO_INCREMENT,
		...
		CREATE TABLE {$wpdb->prefix}ssquiz_history (
			id mediumint(9) UNSIGNED NOT NULL AUTO_INCREMENT,
...
function ssquiz_migrate() {
	...
	foreach ($quizzes as $quiz) {
		...
		$wpdb->insert("{$wpdb->prefix}ssquiz_quizzes", 
			array( 
				'name' => $quiz->title, 
				'type' => $quiz->type, 
				'meta' => serialize( $meta ),
...
function ssquiz_uninstall(){
	...
	//$wpdb->query("
	//	DROP TABLE {$wpdb->prefix}ssquiz_quizzes;
	//	DROP TABLE {$wpdb->prefix}ssquiz_questions;
	//	DROP TABLE {$wpdb->prefix}ssquiz_history;
	//");
}
