...
function ssquiz_crud_quiz(){
	...
	$table = ( $object_type == 'quiz' ) ? "{$wpdb->prefix}ssquiz_quizzes" : "{$wpdb->prefix}ssquiz_questions";
	...
	if( $action == 'reorder' ) {
		...
		if ( $info->start > $info->finish ) {
			...
			$temp = $wpdb->get_var( "SELECT count(*) FROM {$wpdb->prefix}ssquiz_questions WHERE quiz_id = $quiz_id" );
	...
	if( $action == 'delete') {
		if($object_type == 'question') {
			$wpdb->query( "UPDATE {$wpdb->prefix}ssquiz_questions SET number = number - 1 
							WHERE quiz_id = $quiz_id AND number > (SELECT number FROM {$wpdb->prefix}ssquiz_questions WHERE id = $id)" );
		} else { // quiz
			$wpdb->query( "DELETE FROM {$wpdb->prefix}ssquiz_questions WHERE quiz_id = $id;" );
		}
 	...
 	if ( $action == 'read' ) {
		...
		if ( $object_type == 'quiz' ) {
			...
			$temp = explode( "_", $object->type );
			$object->type = $temp[0];
			$object->tag_free = $temp[1];
			if ( ! isset( $object->prerequisites ) ) 
				$object->prerequisites = array();
		} else { // question
			$object->meta = unserialize( $object->meta );
	...
	if($object_type == 'quiz') {
		...
		if( $action == 'edit' && $id < 1 ) {
			...
			if( $info->file > '' )
				ssquiz_import_questions($info->file);
		} else {
	...
	if($object_type == 'question') {
		...
		elseif ( $action == 'edit_new' ) {
			$number = 1 + intval( $wpdb->get_var( "SELECT MAX(number) FROM {$wpdb->prefix}ssquiz_questions WHERE quiz_id = $id" ) );
			$insert = array( 
					'quiz_id' => $id,
					'number' => $number,
					'type' => $type,
...
function ssquiz_import_questions($file) {
	global $wpdb;
	$quiz_id = $wpdb->insert_id;
	$lines = explode("\n", $file);
	for ( $i = 0; $i < count($lines); $i++ ) {
		$line = explode(',', $lines[$i]);
		$wpdb->insert( "{$wpdb->prefix}ssquiz_questions", 
			array( 
				'quiz_id' => $quiz_id, 
				'number' => $i+1,
				'type' => 'fill',
				'question' => $line[0],
				'answers' => serialize( explode( '|', $line[1] ) ) ), 
			array( '%d', '%d', '%s', '%s', '%s' ) );
	}
}
...
function ssquiz_print_questions( $offset, $quiz_id ) {
	...
	$total_questions = $wpdb->get_var( "SELECT count(*) FROM {$wpdb->prefix}ssquiz_questions" );
	$total_quizzes = $wpdb->get_var( "SELECT count(*) FROM {$wpdb->prefix}ssquiz_quizzes" );
	
	$max = 20;
	$temp = ( $quiz_id != 0 ) ? "WHERE quizzes.id = $quiz_id" : '';

	$total_questions_in_list = $wpdb->get_var( "SELECT count(*) 
				FROM {$wpdb->prefix}ssquiz_quizzes as quizzes LEFT JOIN {$wpdb->prefix}ssquiz_questions as questions 
...
function ssquiz_list_quizzes( $all = false ) {
	global $wpdb;
	$quizzes = $wpdb->get_results( "SELECT id, name FROM {$wpdb->prefix}ssquiz_quizzes ORDER BY id ASC" );
