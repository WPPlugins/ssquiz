...
function ssquiz_delete_history(){
	...
	if ( $id == -1 ) {
		$wpdb->query( "DELETE FROM {$wpdb->prefix}ssquiz_history" );
	}
	else if ( $id > 0 )
		$wpdb->query( "DELETE FROM {$wpdb->prefix}ssquiz_history WHERE id = $id" );
...
function ssquiz_print_history( $offset, $filter = '' ) {
	...
	$sql = "
		SELECT count(*)  FROM 
		{$wpdb->prefix}ssquiz_history as history 
		INNER JOIN {$wpdb->prefix}ssquiz_quizzes as quizzes 
		ON (history.quiz_id = quizzes.id)
		LEFT OUTER JOIN {$wpdb->prefix}users as users
	...
	$entries = $wpdb->get_results( "
		...
		FROM {$wpdb->prefix}ssquiz_history as history 
			 INNER JOIN {$wpdb->prefix}ssquiz_quizzes as quizzes 
			 ON (history.quiz_id = quizzes.id)
			 LEFT OUTER JOIN {$wpdb->prefix}users as users
			 ON (history.user_id = users.id)
	...
	foreach ( $entries as $entry ) {
		...
		<li class='ui-state-default row<?php if( $entry->correct == $entry->total ) echo ' ssquiz_right';?>'>
			...
			<div class="sdiv4"><?php echo $entry->name; ?></div>
	...
	<div class="ssquiz_inforamtion">
		<span> Entries:  <?php echo $total_filtered; ?></span>
	<?php
	if ( $filter > '' )
		echo '<span style="margin-left:10px;"> Filtered from: ' . $total . '</span>
	</div>';
}
