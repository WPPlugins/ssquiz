...
function ssquiz_start( $params ) {
	...
	$info->quiz = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}ssquiz_quizzes WHERE id = $quiz_id;");
	$info->questions = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}ssquiz_questions WHERE quiz_id = $quiz_id ORDER BY number ASC;");
	...
	if( $info->one_chance ) {
		$attempts = $wpdb->get_var( "
			SELECT count(*) FROM {$wpdb->prefix}ssquiz_history WHERE user_id = {$info->user->id} AND quiz_id = {$info->quiz->id}
		");
		if( $attempts > 0 )
	...
	if( $current_question->type == 'single' || $current_question->type == 'multi' ) {
		...
		foreach ( $current_question->answers as $answer ) {
			if ($current_question->type == 'single' )
				echo '<input type="radio" name="ssquiz_answer" class="ssquiz_answer" /><span class="ssquiz_answer_span">'
...
function ssquiz_finish( &$finish_screen, &$status, &$info ) {
	...
	$wpdb->insert("{$wpdb->prefix}ssquiz_history", 
					array( 
...
function ssquiz_tag_replace( &$screen, &$info, $state ) {
	$search = array();
	$replace = array();
	$temp_var = ( $info->questions_counter == 0 ) ? -1 : $info->questions_counter;
